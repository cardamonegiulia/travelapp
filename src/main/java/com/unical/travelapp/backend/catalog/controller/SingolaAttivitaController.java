...
@Tag(
        name = "Attività",
        description = "Gestione delle attività turistiche"
)
...
public class SingolaAttivitaController {
    ...
    @Operation(
            ...
            description =
                    "Restituisce tutte le sessioni associate all'attività, " +
                            "necessarie anche per selezionare la sessione da prenotare."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
    ...
    public ResponseEntity<SingolaAttivitaDTO> createAttivitaConSessioni(
            @Valid
            @RequestBody
            SingolaAttivitaRequestDTO attivitaRequest,
            ...
            @RequestParam
            @NotEmpty(
                    message = "Indicare almeno un giorno della settimana"
            )
            @Size(
                    max = 7,
    ...
    @PreAuthorize("hasAnyRole('ORGANIZZATORE', 'ADMIN')")
    @Operation(
            summary = "Aggiunge un'immagine a un'attività"
    )
    ...
    @Operation(
            summary = "Restituisce le immagini di un'attività"
    )
    ...
    @PreAuthorize("hasAnyRole('ORGANIZZATORE', 'ADMIN')")
    @Operation(
            summary = "Rimuove un'immagine da un'attività"
    )
